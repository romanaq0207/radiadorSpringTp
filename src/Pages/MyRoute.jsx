...
function MyRoute() {
  const [ubicacion, setUbicacion] = useState({ lat: null, lon: null });
  const [error, setError] = useState(null);
  const [id_conductor, setIdConductor] = useState(1); // Suponiendo que tienes un id_conductor, cámbialo según sea necesario
        ...
        async (position) => {
          ...
          try {
            await axios.post(`${API_BASE_URL}/ubicacion-conductor`, {
              id_conductor, // Asegúrate de tener el id del conductor correcto
              fecha_hora : new Date().toISOString().slice(0, 19).replace('T', ' '),
              latitud: newLat,
              longitud: newLon
            });
  ...
  }, [id_conductor]); // Dependencia opcional si cambias el id_conductor
