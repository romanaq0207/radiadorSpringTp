...
function MyRoute() {
  ...
  useEffect(() => {
    if ("geolocation" in navigator) {
      // Configurar el temporizador para enviar la ubicación cada 10 segundos
      let intervalId = null;
      ...
      const watchId = navigator.geolocation.watchPosition(
        (position) => {
          ...
          // Si ya existe un interval activo, lo limpiamos antes de crear uno nuevo
          if (intervalId) {
      ...
      // Limpiar el intervalo y el watchPosition cuando el componente se desmonte
      return () => {
        if (intervalId) {
          clearInterval(intervalId); // Detener el intervalo al salir de la página
        }
        navigator.geolocation.clearWatch(watchId); // Limpiar el watchPosition
      };
    } else {
      setError("La geolocalización no es soportada por este navegador.");
    }
