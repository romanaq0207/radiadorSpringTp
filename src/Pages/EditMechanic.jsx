...
function EditMechanic() {
    ...
    useEffect(() => {
        axios.get(`${API_BASE_URL}/mecanicos/${id}`)
            .then(response => setMecanicoData(response.data))
            .catch(error => console.error('Error al cargar los datos del mecánico:', error));
    }, [id]);
    ...
    const handleSave = () => {
        ...
        axios.put(`${API_BASE_URL}/mecanicos/${id}`, mecanicoData)
            .then(response => {
                console.log('Mecánico actualizado:', response.data);
                navigate('/'); // Redirige a la página principal después de actualizar
            })
            .catch(error => console.error('Error al actualizar el mecánico:', error));
    };
